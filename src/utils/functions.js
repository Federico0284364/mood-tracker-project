...
export function getSleepRange(hours){
  ...
  export function calculateAverageSleep(recordList){
    const last5Entries = recordList.slice(recordList.length - 5);
    const hours = last5Entries.map((entry) => { return getSleepHours(entry.sleep)});
    const totalHours = hours.reduce((accumulator, entry) => {
      return accumulator + entry
    })
    const averageHours = totalHours/hours.length;
    const averageRange = getSleepRange(averageHours);
